  ...
  return ( 
    <div>
      ...
      <Row className='pt-1'>
        ...
        <Col className='px-1 mt-1'>
          <Card>
            <Card.Header className='fw-bold fs-6 text-center'>Superliga</Card.Header>
            <Card.Body>
              <Table responsive bordered size='sm'>
                <tbody>
                  <tr>
                    <td colSpan={12} className='text-center text-light fw-bold bg-success align-middle'>
                      <p className='m-0' style={{'fontSize': '1.2rem'}}>{`GREENS: ${superliga[1][0]} (${((superliga[1][0] * 100) / 480).toFixed(2)}%)`}</p>
                    </td>
                    <td colSpan={12} className='text-center text-light fw-bold bg-danger align-middle'>
                      <p className='m-0' style={{'fontSize': '1.2rem'}}>{`REDS: ${superliga[1][1]} (${((superliga[1][1] * 100) / 480).toFixed(2)}%)`}</p>
                    </td>
                  </tr>
                  <tr>
                    <td className='text-center text-light fw-bold bg-secondary align-middle'>
                      ...
                      return (
                        <tr key={index1} className='text-center'>
                          ...
                          jogos.map((jogo, index2) => {
                            ...
                            return (
                              <td key={`4${index1}${index2}`} className={'text-light hoverChange align-middle'} style={{ backgroundColor: jogo.classe}}>
                                <OverlayTrigger
                                  placement="top"
                                  delay={{ show: 250, hide: 400 }}
                                  overlay={
                                ...
                                >
                                  <div id={`4${index1}${index2}`} className='placarsuper' onClick={() => atualizaSelecao(`${casaFinal === '5+' ? '5' : casaFinal}x${foraFinal === '5+' ? '5' : foraFinal}`, `4${index1}${index2}`, 'super')}>
                                    <p className='m-0' style={{'fontSize': '0.8rem'}}>{`${casaFinal === '5+' ? '5' : casaFinal}x${foraFinal === '5+' ? '5' : foraFinal}`}</p>
                                    <p className='m-0' style={{'fontSize': '0.7rem'}}>{ mostrarPlacar ? out ? 'oth' : `${casaIntervalo}x${foraIntervalo}` : '-'}</p>
                                  </div>
                                </OverlayTrigger>
